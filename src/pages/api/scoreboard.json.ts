...
export const GET: APIRoute = async () => {
    ...
    return json(
      {
        ...
        source: 'fallback',
        message: 'Set FOOTBALL_DATA_TOKEN in your environment to enable latest soccer scores.',
        matches: [],
      },
      'no-store',
    );
  ...
  try {
    ...
    const weekAgo = getDateDaysAgoInNewYork(7);
    const matches = await fetchMatches(
      token,
      `/competitions/${WORLD_CUP_COMPETITION}/matches?dateFrom=${weekAgo}&dateTo=${today}`,
    );
    ...
    const payload: ScoreboardResponse = {
      ...
      message:
        selectedMatches.length === 0
          ? `No World Cup matches found from ${weekAgo} to ${today}.`
          : undefined,
    ...
    const message = error instanceof Error ? error.message : 'Unable to load scoreboard.';
